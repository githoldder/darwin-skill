...
![Darwin Skill logo](assets/logo.svg)

`darwin-skill` 是一个面向 AI Agent 的本地 Skills 进化插件。它不是一组普通提示词，而是一套用于扫描、整理、蒸馏、审查、脚本化和归档本机 skills / prompts / SOP 资产的工具包。

当前版本：`v1.0.0`

## 用途

当你的本地 skills、提示词、工作流和项目 SOP 越积越多时，Agent 很容易陷入两种浪费：

- 把所有内容都塞进上下文，导致注意力稀释。
- 反复重新解释同一套工作流，导致 token 和时间浪费。

达尔文.skill 的目标是让 Agent 持续判断：
